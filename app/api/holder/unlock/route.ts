...
import { holderConfigured, getShare, serveShare, isRevoked } from "@/lib/holder-store"
...
export async function POST(request: Request) {
  ...
  const result = await resolveUnlock(entityKey, authKey, { getGrant, getShare, serveShare, isRevoked })
  if (!result.ok) {
